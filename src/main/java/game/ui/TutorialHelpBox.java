...
public class TutorialHelpBox implements Observer {
    ...
    public TutorialHelpBox(SpriteBatch batch) {
        ...
        for (int i = HelpBox.text.length - 1; i >= 0; --i) {
            forward.push(HelpBox.text[i]);
        }
        text = new BitmapFont();
    ...
    public void update() {
        ...
        glyphLayout.setText(text, forward.isEmpty() ? "" : forward.peek(), Color.BLACK,
                            336, Align.center, true);
        text.draw(batch, glyphLayout, 64, 218);
        batch.end();
